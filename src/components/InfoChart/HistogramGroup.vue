<template>
  <div id="chartGroup">
    <ul class="picGroup" style="display: flex">
      <li
        v-for="(item, index) in issusLineData"
      ...
      >
        <el-card class="oneChart" style="padding: 2px">
          <div :id="getDivID(index)" style="width: 240px; height: 170px"></div>
        </el-card>
...
export default {
  ...
  computed: {
    ...mapState({
      issusLineData: (state) => state.graph.issusLineData,
    }),
  },
  watch: {
    // 问题清单列表数据
    issusLineData: {
      handler(newval, oldval) {
        ...
        if (newval.length > 0) {
          this.$nextTick(() => {
            console.log("----issusLineData", this.issusLineData);
            this.issusLineData.forEach((item, index) => {
            ...
            //构建单个图
            this.initOneLineChart(0, this.issusLineData[0]);
          });
        }
      },
    ...
    initHistogram() {
      ...
      let option = {
        ...
        legend: {
          type: 'scroll',
          // orient: 'vertical',
          data: this.issusLineData.legendData,
          align: "right",
          right: 10,
          textStyle: {
            color: "#333",
      ...
      this.issusLineData.seriesData.forEach((item) => {
        let obj = {
          name: item.name,
          type: "bar",
          data: item.data,
