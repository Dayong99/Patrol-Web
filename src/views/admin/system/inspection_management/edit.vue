<template>
  <div>
    <el-dialog
      :width="width"
      top="50px"
      custom-class="management_editdlg"
...
export default {
  ...
  methods: {
    ...
    setInspection(row) {
      this.newInspection.id = row.main.id;
      this.newInspection.patrolUnitId = row.maps.id
      this.newInspection.session = row.main.session;
      this.newInspection.round = row.main.round;
      this.newInspection.year = row.main.year;
      this.newInspection.groupName = row.main.groupName;
      this.newInspection.unitName = row.unit.name;
      this.newInspection.patrolTime = row.main.patrolTime;
      this.newInspection.endTime = row.main.endTime;
    },
    ...
    submitForm() {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          // console.log("newInspection:",this.newInspection);
          await this.$put("/inspection/patrol", {
            endTime: this.newInspection.endTime,
            id: this.newInspection.id,
            patrolTime: this.newInspection.patrolTime,
            // round: this.newInspection.round,
            // session: this.newInspection.session,
            // year: this.newInspection.year,
          })

          this.$put("/inspection/patrol/updateGroup", { ...this.newInspection }).then(() => {
