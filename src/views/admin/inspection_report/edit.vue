    ...
    >
      ...
      <div class="dialog_content">
        ...
        >
          ...
          <!-- <el-form-item label="报告上传时间" prop="uploadtime">
            <el-date-picker
              v-model="onespecial.uploadtime"
              type="datetime"
              placeholder="选择日期时间"
              value-format="yyyy-MM-dd HH:mm:ss"
            >
...
export default {
  ...
  methods: {
    ...
    getPatrol(val) {
      ...
      this.onespecial.unitName = val.unit.name;
      this.onespecial.groupName = val.groupName;
      this.onespecial.patrolTime = val.patrolTime;
      this.onespecial.endTime = val.endTime;
      this.onespecial.patrolUnitId = val.reportPatrolUnit.id;
      this.onespecial.type = 1;
      this.onespecial.uploadtime = val.reportPatrolUnit.report.patrolTime;
      ...
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          this.$put("/inspection/t-report", {
            id: this.report.id,
            patrolId: this.report.patrolId,
            patrolTime: this.onespecial.uploadtime,
          }).then((r) => {
            if (r.data.code === 20000) {
              this.$message.success("修改成功");
              this.$emit("success");
