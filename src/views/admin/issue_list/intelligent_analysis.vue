...
import Histogram from "@/components/InfoChart/HistogramNum.vue";
import HistogramGroup from "@/components/InfoChart/HistogramGroup.vue";
export default {
  name: "clueAnalysis",
  ...
  data() {
    return {
      ...
      colorList1: ['#5b8ff9', '#5ad8a6', '#5d7092', '#5b8ff9', '#5ad8a6', '#5d7092', '#5b8ff9', '#5ad8a6', '#5d7092'],
      // colorList1: [
      //   "#5B8FF9",
      //   "#5D7092",
      //   "#5AD8A6",
  ...
  watch: {
    ...
    "queryParams.first"(val) {
      ...
      if ((val == "" || val == null) && this.queryParams.year == "") {
        this.initHistogram(this.chartFlag);
      }
    },
  ...
  methods: {
    ...
    // },
    ...mapMutations({
      setIssusBarGraphData: "graph/setIssusBarGraphData",
      setIssusLineData: "graph/setIssusLineData",
    }),
    ...
    initHistogramByYear(flag) {
      ...
      this.$get("/inspection/t-problem/analysis", {
        ...
        this.setIssusBarGraphData(optionsData);
      });
    },
    ...
    async initLineChart() {
      ...
      let labelNameArr = [];
      try {
        let counts = await this.$get("inspection/t-problem/counts", {
          session: this.queryParams.session,
          sort: this.queryParams.sort,
          // round: this.queryParams.round,
          // firstid: this.queryParams.first,
        ...
        let analysis = await this.$get("/inspection/t-problem/analysis", {
          session: this.queryParams.session,
          sort: this.queryParams.sort,
          // round: this.queryParams.round,
        ...
        for (let i = 0; i < labelNameArr.length; i++) {
          ...
          let detaildata = await this.$get("/inspection/t-problem/analysis", {
            session: this.queryParams.session,
            sort: this.queryParams.sort,
            firstId: labelNameArr[i].id,
          });
        ...
        if (LineChartData.length > 0) {
          ...
          this.setIssusLineData(LineChartData);
        } else {
          this.$message.warning("当前没有分析数据");
        }
      } catch (error) {
    ...
    initChartMap() {
      ...
      let chartMap = this.$echarts.init(document.getElementById("chart_map"))
      let option = null
