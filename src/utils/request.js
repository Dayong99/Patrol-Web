...
// 系统全局请求对象
const service_json = axios.create({
...
service_json.interceptors.request.use(
  config => {
    let _config = config
    try {
...
service_json.interceptors.response.use((config) => {
  return config
}, (error) => {
  if (error.response) {
...
const request = {
  ...
  login(url, params) {
    ...
    return service_json.post(url, params, {
      transformRequest: [(params) => {
        return tansParams(params)
      }],
      headers: {
  ...
  put(url, params) {
    return service_json.put(url, params, {
      transformRequest: [(params) => {
        return tansParams(params)
      }],
      headers: {
  ...
  get(url, params) {
    ...
    return service_json.get(`${url}${_params}`)
  },
  delete(url, params) {
    let _params
    if (Object.is(params, undefined)) {
    ...
    return service_json.delete(`${url}${_params}`)
  },
  getbuffer(url, params) {
    ...
    return service_json.get(`${url}${_params}`, { responseType: 'arraybuffer' })
  },
  ...
  downloadGet(url, params, filename) {
    ...
    return service_json.get(`${url}${_params}`, {responseType: 'blob'}).then(r => {
      const content = r.data
      const blob = new Blob([content])
      if ('download' in document.createElement('a')) {
        const elink = document.createElement('a')
  ...
  downloadZip(url, params, fileName) {
    ...
    return service.get(`${url}${_params}`, {responseType: 'blob'}).then(r => {
      const data = r.data
